...
const page = usePage();
const flashMessage = ref(page.props.flash.error);
const form = useForm({
    tmdb_id: '',
});
...
<template>
    ...
    <AdminAuthenticatedLayout>
        <template #header>
            <PageHeader LinkText="Casts Index " HeadLink="Create Casts" :route="route('admin.casts.index')" />
        </template>
        <div class="py-2">
                                ...
                                <div class="w-full">
                                    <div class="">
                                        <label for="first-name" class="block text-sm font-medium text-gray-700">Cast
                                            Tmdb Id</label>
                                        <input v-model="form.tmdb_id" type="text" autocomplete="given-name"
