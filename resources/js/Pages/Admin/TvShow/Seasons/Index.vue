...
function deleteTv(seasonId) {
    router.delete(route("admin.season.destroy", seasonId), {
        onSuccess: () => {
            flashMessageSuccess.value = "Tv deleted successfully.";
            closeModal();
        }
    });
}
...
function generateSeason() {
    router.post(route("admin.season.store", props.tvShow), {
    ...
    <AdminAuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Season Index</h2>
            </div>
        </template>
                    ...
                    <section class="container mx-auto p-6 font-mono">
                        <div class="flex justify-end items-center">
                            <div class="flex items-center mb-5 bg-white p-3">
                                ...
                                <label for="" class="form-label me-3">Season Id</label>
                                <input type="text" name="" v-model="season_number" id="" class="form-control rounded-lg"
                                    placeholder="Season Tmdb Id" aria-describedby="helpId" />
                                <small id="helpId" v-if="props.errors" class="text-red-500 block">Help
                                    text</small>
                                <button @click="generateSeason"
                        ...
                        <div class="w-full mb-8 overflow-hidden bg-white rounded-lg shadow-lg">
                            <div class="p-2 m-2">
                                <div class="flex justify-between">
                                    ...
                                    <div class="flex">
                                        <select v-model="prePage" @change="getSeasons"
                            ...
                            <div class="w-full overflow-x-auto">
                                <Table>
                                    <template #tableHead>
                                        ...
                                        <TableHead class="">Manage</TableHead>
                                    </template>
                                    <TableRow v-for="(season, i) in seasons.data" :key="i">
                                        <TableData>{{ season.name }}</TableData>
                                        <TableData>{{ season.slug }}</TableData>
                                        <TableData>{{ season.season_number }}</TableData>
                                        <TableData>
                                            <img :src="`https://www.themoviedb.org/t/p/w220_and_h330_face/${season.poster_path}`"
                                        ...
                                        <TableData>
                                            ...
                                            Episodes</Link>
                                            <Link :href="route('admin.season.edit', [tvShow.id, season.id])"
                                            ...
                                            <button
                                                ...
                                                @click="openModal(season)">Delete</button>
                                        </TableData>
                                    </TableRow>
                                </Table>
                        ...
                        <MainModal :show="showModal" @close="closeModal">
                            ...
                            <template #footer>
                                <button @click="deleteTv(selectedSeason)"
