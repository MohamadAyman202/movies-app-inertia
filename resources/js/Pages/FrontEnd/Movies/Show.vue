    ...
    <AuthenticatedLayout>
        <main class="my-2" v-if="movie">
                            ...
                            <div class="m-4 p-6">
                                ...
                                <div class="flex p-3 text-white space-x-4">
                                    ...
                                    <span class="ml-2 space-x-1">
                                        <Link v-for="genre in genres" :key="genre.slug"
                                ...
                                <div class="flex space-x-4">
                                    <button @click="openModal(trailers)"
                                        class="rounded-md bg-black/20 px-4 py-2 text-sm font-medium text-white hover:bg-black/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75"
                                        v-for="trailer in trailers" :key="trailer.slug">
                                        {{ trailer.name }}
                                    </button>
                                </div>
                            </div>
                ...
                <div class="flex justify-between">
                    <div class="w-7/12">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
                            <MovieCard v-for="cast in movie.casts" :key="cast.slug">
                                <template #image>
            ...
            <section class="max-w-6xl mx-auto bg-gradient-to-r from-indigo-700 to-transparent mt-6 p-2"
                v-if="movie.tags">
                <span v-for="tag in movie.tags" :key="tag.slug"
                    class="font-bold text-white hover:text-indigo-200 mx-2 select-none">
                        {{tag.tag_name}}
                </span>
            </section>
        </main>
